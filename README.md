# Ekken
*Efficient Knowledge Kinetic Execution Node*  
*Inspired by Ekkenshiki (謁見式) — a formal audience.*

**Ekken** is a **Workflow Platform** built with **Go** and **Vue** — designed for high-concurrency execution with minimal overhead. Potato-friendly 🥔, no cloud bullshit, no data sneaking.

### Goal

Bridge the gap between **manual precision** and **autonomous intelligence**. Most automation tools are either too rigid or too unpredictable. Ekken aims to be neither.

- [x] **Single Binary:** No external dependencies. Lightweight, fast, stays out of your way.
- [x] **Full Control:** A purpose-built editor for those who know exactly what they want.
- [ ] **True Agnostic:** Swap providers or models freely — tool-call normalization across providers, no lost context.
- [ ] **Assistant as Engineer:** Describe the chore, review the generated proposal, approve to save — then run it.
    - [x] Searches and stitches internal nodes.
    - [ ] Fetches from Plugin Hub when needed.
    - [ ] Synthesizes new nodes on-the-fly when the library falls short.🙂‍↕️
- [x] **Plugin Hub:** Remote fetching and management of curated, verified nodes.

---

> Slow development. MVP is NOT coming soon 🤦🏻‍♂️  
> *Epriting ken bi ceng.* 🏴‍☠️